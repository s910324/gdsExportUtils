...
def gdsFilesSummary(fileFullPathList):
    ...
    for fileFullPath in fileFullPathList:
        ...
        layoutDUB    = round(layout.dbu, 8)
        checkSum     = cksum(fileFullPath)
        fileStats    = os.stat(fileFullPath)
        topCellList  = [layout.cell(i) for i in layout.each_top_cell()]
        fileSize     = f"{fileStats.st_size:,}"
        
        for cell in topCellList:
            cellName   = cell.name
            cellBox    = cell.dbbox()
            cellWidth  = round(cellBox.width(), 8)
            cellHeight = round(cellBox.height(), 8)
            cellP1x    = round(cellBox.p1.x, 8)
            cellP1y    = round(cellBox.p1.y, 8)
            cellP2x    = round(cellBox.p2.x, 8)
            cellP2y    = round(cellBox.p2.y, 8)
            cellLLC    = f"({cellP1x}, {cellP1y})"
            cellURC    = f"({cellP2x}, {cellP2y})"
            cadWindow  = f"{cellLLC}, {cellURC}"
            cellOrigin = "Non Standard"
            ...
            result.append(
                f"{fileName}|{checkSum}|{fileSize}|{layoutDUB}|{cellName}|{cadWindow}|{cellWidth}|{cellHeight}|{cellOrigin}"
            )
    return ("\n".join(result))
    
def gdsInfo(fileFullPath):
    layoutView   = pya.LayoutView()
    cellView     = layoutView.cellview(layoutView.load_layout(fileFullPath))
    layout       = cellView.layout()
    ...
    fileName     = os.path.basename(fileFullPath)
    layerInf     = "; ".join([f"L({inf.layer},{inf.datatype})" for inf in layout.layer_infos()])
    layerCount   = len(layout.layer_infos())
    layoutDUB    = round(layout.dbu, 8)
    checkSum     = cksum(fileFullPath)
    fileStats    = os.stat(fileFullPath)
    topCellList  = [layout.cell(i) for i in layout.each_top_cell()]
    topCellNames = "; ".join([layout.cell(i).name for i in layout.each_top_cell()])
    rAlign       = 4
    lAlign       = 18
    fileInfoList = [
            f"{bullet}{str(index).rjust(5)}| {item}" for index, item in enumerate([
                "File path"      .ljust(lAlign) + f"| {fileFullPath}",
                "File name"      .ljust(lAlign) + f"| {fileName}",
                "Top cell list"  .ljust(lAlign) + f"| {topCellNames}",
                "File Cksum"     .ljust(lAlign) + f"| {checkSum}", 
                "File size(byte)".ljust(lAlign) + f"| {fileStats.st_size:,}",
                "File size(KB)"  .ljust(lAlign) + f"| {round(fileStats.st_size/1024.0):,}",
                "File size(MB)"  .ljust(lAlign) + f"| {round(fileStats.st_size/1024.0/1024.0):,}",
                "Layer count"    .ljust(lAlign) + f"| {layerCount}",
                "Layer Info"     .ljust(lAlign) + f"| {layerInf}",
                "Database unit"  .ljust(lAlign) + f"| {layoutDUB}",
            ])
    ]

    
    cellInfoList = []
    for i, cell in enumerate(topCellList):
        cellInfoList.append(cellInfo(cell, prefix = f"{bullet}{len(fileInfoList)-1}.{i+1}."))
...
def cellInfoDict(cell):
    ...
    return {
        ...
        "cellLLC"    : f"({round(cellBox.p1.x, 8)}, {round(cellBox.p1.y, 8)})",
        "cellURC"    : f"({round(cellBox.p2.x, 8)}, {round(cellBox.p2.y, 8)})",
        "boxCenter"  : (round(cellBox.center().x, 8), round(cellBox.center().y, 8)),
        "stdCenter"  : {
            (           0,             0) : "cell center", 
            ( cellWidth/2,  cellHeight/2) : "lower left",
            (-cellWidth/2, -cellHeight/2) : "upper right",
        },
    }
    
def cellInfo(cell, prefix = ""):
    info       = cellInfoDict(cell)
    origin     = "Non Standard" if not(info["boxCenter"] in info["stdCenter"]) else info["boxCenter"]
    warning    = "" if not(origin == "cell center") else "Non-Centered Origin will caused issue if layout filpped."    
    rAlign     = 8
    lAlign     = 18
    cellInfo = "\n".join([
            f"{(prefix+str(index + 1)).rjust(rAlign)}| {item}" for index, item in enumerate([
                "Cell name"    .ljust(lAlign) + f"| {info['cellName']}",
                "CAD window"   .ljust(lAlign) + f"| {info['cellLLC']}, {info['cellURC']}",
                "CAD size(W/H)".ljust(lAlign) + f"| {info['cellWidth']} x {info['cellHeight']}",
                "CAD origin"   .ljust(lAlign) + f"| {origin}",
                "Warning"      .ljust(lAlign) + f"| {warning}",
            ])
    ])
    return cellInfo
    
    
def listgds(folderPath):
    return [os.path.join(folderPath, f) for f in os.listdir(folderPath) if (os.path.isfile(os.path.join(folderPath, f)) and ( f.split(".")[-1] == "gds"))]
